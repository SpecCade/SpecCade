...
pub struct TileableWorleyNoise {
    inner: WorleyNoise,
    period_x: i32,
    period_y: i32,
}

impl TileableWorleyNoise {
    /// Create a new tileable Worley noise generator.
    pub fn new(seed: u32, period_x: i32, period_y: i32) -> Self {
        Self {
            inner: WorleyNoise::new(seed),
            period_x,
            period_y,
        }
    }
    ...
    pub fn with_jitter(mut self, jitter: f64) -> Self {
        self.inner = self.inner.with_jitter(jitter);
        self
    }

    /// Set the distance function.
    pub fn with_distance_function(mut self, func: DistanceFunction) -> Self {
        self.inner = self.inner.with_distance_function(func);
        self
    }

    /// Set the return type.
    pub fn with_return_type(mut self, return_type: WorleyReturn) -> Self {
        self.inner = self.inner.with_return_type(return_type);
        self
    }

    /// Hash function for cell coordinates with wrapping.
    fn cell_point(&self, cell_x: i32, cell_y: i32) -> (f64, f64) {
        // Wrap cell coordinates
        let wrapped_x = ((cell_x % self.period_x) + self.period_x) % self.period_x;
        let wrapped_y = ((cell_y % self.period_y) + self.period_y) % self.period_y;

        // Get the base point from the wrapped cell
        let (base_px, base_py) = self.inner.cell_point(wrapped_x, wrapped_y);

        // Offset to the actual cell position
        let offset_x = cell_x - wrapped_x;
        let offset_y = cell_y - wrapped_y;

        (
            base_px - wrapped_x as f64 + cell_x as f64,
            base_py - wrapped_y as f64 + cell_y as f64,
        )
    }
}

impl Noise2D for TileableWorleyNoise {
    fn sample(&self, x: f64, y: f64) -> f64 {
        // Find the cell containing the point
        let cell_x = WorleyNoise::fast_floor(x);
        let cell_y = WorleyNoise::fast_floor(y);
        ...
        for dy in -1..=1 {
            for dx in -1..=1 {
                ...
                let (px, py) = self.cell_point(cx, cy);
                let dist = self.inner.distance(x, y, px, py);
        ...
        // Return based on return type
        let result = match self.inner.return_type {
            WorleyReturn::F1 => f1,
            WorleyReturn::F2 => f2,
            WorleyReturn::F2MinusF1 => f2 - f1,
            WorleyReturn::F1PlusF2 => (f1 + f2) * 0.5,
